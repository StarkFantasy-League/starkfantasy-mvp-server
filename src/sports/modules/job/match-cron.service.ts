...
export class TaskMatchService {
  ...
  async updateMatchsForSeason() {
    try {
      ...
      for (const match of matches) {
        ...
        const exists = await this.cricketMatchService.findOne(matchId).catch(() => null);
        const localteamId= match.localteam_id.toString();
        const visitorteamId= match.visitorteam_id.toString();
        if (!exists) {
         const cricketMatch = new CricketMatch(matchId, localteamId, visitorteamId, match.starting_at); 
          await this.cricketMatchService.create(cricketMatch);
          this.logger.log(`🟢 Partido insertado de: ${match.localteam_id} vrs ${match.visitorteam_id}`);
        } else {
      ...
      this.logger.error('❌ Error al obtener partidos IPL desde Sportmonks:', error);
    }
  }
  ...
  async deleteMatchs() {
    ...
    try {
      ...
      this.logger.log(`✅ Todos los partidos han sido eliminados (${allMatchs.length})`);
    } catch (error) {
