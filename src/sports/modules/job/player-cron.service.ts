...
export class TaskPlayerService {
  ...
  async UpdatePlayersBySeason() {
    try {
      const players = await this.sportmonksService.getAllPlayersFromStoredTeams();
      this.logger.log(`✅ Jugadores IPL obtenidos (${players.length}):`);

      for (const player of players) {
        
        const playerId = player.id.toString();
        const exists = await this.cricketPlayerService.findOne(playerId).catch(() => null);
        const teamId= player.teamId.toString();
        if (!exists) {
         const cricketPlayer = new CricketPlayer(playerId,teamId, player.firstname,player.lastname,player.positionName,player.image_path); 
      ...
      this.logger.error('❌ Error al obtener jugadores IPL desde Sportmonks:', error);
    }
  }
  ...
  async deletePlayersForNewSeason() {
    ...
    try {
      ...
      this.logger.log(`✅ Todos los jugadores han sido eliminados (${allPlayers.length})`);
    } catch (error) {
