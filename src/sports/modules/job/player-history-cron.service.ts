  ...
  async generatePlayerHistory() {
    try {
      ...
      const playerTotals = new Map<
        number,
        { runs: number; wickets: number; catches: number; points: number }
      >();
      for (const [weekKey, weekMatches] of Object.entries(matchesByWeek)) {
        ...
        for (const match of weekMatches) {
          const matchDetails =
            await this.sportmonksService.getPerformancesByMatch(match.id);
          matchDetails.batting?.forEach((b) => {
            ...
            const existing = playerWeekly.get(playerId) || {
              runs: 0,
              wickets: 0,
              catches: 0,
              points: 0,
      ...
      for (const [playerId, stats] of playerTotals.entries()) {
        ...
        this.logger.log(
          `📥 Historial creado para el jugador ${playerId} con ${stats.points} puntos`,
        );
      }
    } catch (error) {
